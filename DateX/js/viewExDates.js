...
$(document).ready(function(){
     ...
     $('#change_sup').unbind().on('click', function(){
         ...
         window.location = 'viewExDates.php?pt=' + supplier;
     });
     ...
     $('#change_docid').unbind().on('click', function () {
        ...
        //  }
        if (Number.isInteger(docid_filter)) {
            window.location = 'viewExDates.php?docid=' + docid_filter + '&pt=' + supplier;        
        }
     });
     ...
     $('#change_date').unbind().on('click', function () {
        ...
        if (Date.parse(date_filter)) {
            window.location = 'viewExDates.php?date=' + date_filter;
        }
     });

     $(".docid, .datepicker").unbind().on('keydown', function(e){
     ...
     $('.edit').unbind().on('click', function(e){
         if($(this).text() == 'Επεξεργασία' && edit == 0){
            console.log('edit mode '+ $(this).text());
            //  e.preventDefault();
             edit = 1;
             row = $('.edit').index(this)+1; //get the index of the element to know which row we are processing
             $(this).attr('class', 'store'); //change class to .insert  
             $(this).text('Αποθήκευση'); //change text to 'save'

             $('.row' + row + ' input').prop('disabled', false); //enable editing for all the input elements in this row
             background = $('.row' + row).css('background-color'); //save current row bg color
             $('.row' + row + ', .row' + row + ' input').css('background-color', '#a5c7fe'); //change row bg color so that user knows which row is on edit
            ...
            //  blockUser(row);
             ...
             $('.row' + row + ' input').unbind().on('keydown', function(k){ //detect keyboard input
                 if(k.which == 27){ //if ESC key is pressed, the function cancels all edits made
                     ...
                     $('.store').text('Επεξεργασία'); //revert to original text of 'EDIT'
                     $('.store').attr('class', 'edit'); //restore original className of .edit
         ...
         else if ($(this).text() == 'Επεξεργασία' && edit == 1) {
             return false;
         }
         else{
             storeClick();
     ...
     function storeClick() {
         ...
         row_newvals = [];
         $('.row' + row + ' .store').text('Επεξεργασία'); //revert to original text of 'EDIT'
         $('.row' + row + ' .store').attr('class', 'edit'); //restore original className of .edit
         $('.row' + row).css('background-color', background); //restore previous row bg color
         edit = 0;
        //  unBlockUser(row);
     }
