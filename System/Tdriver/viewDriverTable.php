...
<?php
	$query = "SELECT * FROM driver";  //You don't need a ; like you do in SQL
	$result = mysqli_query($conn, $query);

	echo "<table id='dtable'>"; // start a table tag in the HTML
	...
	if(@mysqli_num_rows($result)>0){ //throws warning if table is empty - @<function> hides it
		while($row = mysqli_fetch_array($result)){ //Creates a loop to loop through results
			...
			echo "<tr>
					...
					<td class='c0'><p>" . $row['idDriver'] . "</p></td>
					<td class='c1'><p>" . $row['DName'] . "</p></td>
					<td class='empty_cell'></td>
				</tr>";
		}
	}
	...
	$tableCols = mysqli_num_rows(mysqli_query($conn, 'DESCRIBE driver'));
?>

		<tr id='add_new'>
			<form id='add1' action='insertIntoDriverTable.php' method='post'>
				<td class='empty_cell'></td> <!-- 'delete-button' column -->
				<td class='last_row_cell'>
					<input type='text' name='idDriver' id='txtcell' value = '<?php print_r((int)($max+1)) ?>' style='text-align:center'>
				</td>
				<td class='last_row_cell'>
					<input type='text' name='DName' id='txtcell'>
				</td>
