$(document).ready(function() {
		...
		if($('#drop').val()>0 && $('#datepicker').val()!=null && $.trim($('#datepicker').val())!=''){
			...
			$.ajax({
				...
				success: function(data){
                    urlPath = 'viewResList.php?ex='+exc+'&date='+date;
	...
	$('#addRes-btn').on('click', function(){		
		$(this).attr( "href",  "addReservation.php?ex="+$('#drop').val()+"&date="+$.trim($('#datepicker').val()));
	});
    ...
    function calendarSetup(){
        ...
        $("#datepicker").datepicker({dateFormat : 'dd-mm-yy', beforeShowDay: function(date){ //attach calendar to input with given specs
            day = date.getDay();
    ...
    function processAjaxData(data, urlPath){
     ...
     window.history.pushState({"html":data,"pageTitle":"Reservations"},"", urlPath);
    }
